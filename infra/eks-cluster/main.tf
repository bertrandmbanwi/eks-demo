...
module "kms" {
  ...
  version = "1.5.0" # Note - be mindful of Terraform/provider version compatibility between modules
...
locals {
  ...
  kubeconfig = templatefile("${path.module}/templates/kubeconfig.tpl", {
    ...
    aws_authenticator_env_variables   = {AWS_PROFILE = var.aws_profile} })
}
...
/*resource "kubernetes_config_map" "aws_auth_file" {
...
resource "kubernetes_config_map_v1_data" "aws_auth_data" {
  ...
  depends_on = [
    ...
    kubernetes_config_map.aws_auth_file,
  ]
}
*/
provider "helm" {
